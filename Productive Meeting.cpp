...
//    for(auto itr:ms) cout<<itr.fi<<" "<<itr.se<<"\n";
//    nl
    vpll tmp;
    while(s.sz>1){
        auto itr1 = *s.rbegin();
        ...
        itr1.fi--;
        itr2.fi--;
        tmp.pb({itr1.se,itr2.se});
        if(itr1.fi) s.insert(itr1);
        if(itr2.fi) s.insert(itr2);
    }
    cout<<tmp.sz;
    nl
    rep(i,tmp.sz) cout<<tmp[i].fi+1<<" "<<tmp[i].se+1<<"\n";
}
